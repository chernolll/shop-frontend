...
Admin 端审核中心，集中处理 BD 提交的各项申请：预算、汇款、公开任务申请。
...
> **重构变化:** 达人筹备审核（`/review/kol-prepare`）已移除。BD 达人筹备不再需要 Admin 审核流程。
